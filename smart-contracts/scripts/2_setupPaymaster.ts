...
(async () => {
  try {
    ...
    const entryPoint = await ethers.getContractAt("IEntryPoint", config[network].entryPoint, deployer);
    const fiatPaymaster = await ethers.getContractAt("FiatPaymaster", config[network].fiatPaymaster, deployer);
